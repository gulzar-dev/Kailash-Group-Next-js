...
export const Hero = ({ data, onExplore, onContact }) => {
  const hero = data || defaultHero;
  const bgImage = hero.backgroundImage
    ? urlFor(hero.backgroundImage).width(1920).quality(80).url()
  ...
  return (
    <section ref={ref} data-testid="hero-section" className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <motion.div style={{ y: imgY, scale: imgScale }} className="absolute inset-0 -z-10">
        <img src={bgImage} alt={hero.imageAlt || "Australian skyline"} className="w-full h-[120%] object-cover" />
      ...
      >
        <h1 className="font-display font-bold text-5xl sm:text-6xl lg:text-[5.5rem] leading-[1.08] tracking-tight text-[#0A2540] max-w-5xl mt-4">
          <MaskLines delay={0.4} lines={[hero.heading1]} />
