...
export const Awards = () => (
  <section id="awards" data-testid="awards-section" className="relative z-10 bg-white py-28 md:py-40 overflow-hidden">
    <div className="max-w-[1400px] mx-auto px-6 sm:px-12">
      <Reveal className="max-w-2xl mb-16">
        <div className="overline mb-5">Awards & Recognition</div>
      ...
      <div className="flex gap-6 overflow-x-auto px-6 sm:px-12 pb-8 snap-x [scrollbar-width:none] [-ms-overflow-style:none]" style={{ scrollbarWidth: "none" }} data-testid="awards-track">
        {AWARDS.map((a, i) => (
          <motion.div
            ...
            className="snap-start shrink-0 w-[300px] relative rounded-2xl border border-[#e6e3da] bg-[#faf9f6] p-8 overflow-hidden group"
          >
            {/* champagne glow */}
            ...
            <div className="relative">
              <div className="w-14 h-14 rounded-full bg-[#111] flex items-center justify-center mb-8 shadow-[0_10px_30px_rgba(184,145,47,0.3)]">
                <Trophy className="text-champagne" size={24} strokeWidth={1.5} />
              </div>
              <div className="font-display text-5xl text-[#111]">{a.year}</div>
              <h3 className="mt-4 font-medium text-lg text-[#111] leading-snug">{a.title}</h3>
              <p className="mt-2 text-sm text-[#6b6b6b] font-light">{a.org}</p>
            </div>
          </motion.div>
        ))}
      </div>
    ...
    <div className="max-w-[1400px] mx-auto px-6 sm:px-12 mt-4">
      <span className="text-xs text-[#8a8a8a]">← Scroll to explore the full timeline</span>
    </div>
  </section>
);
